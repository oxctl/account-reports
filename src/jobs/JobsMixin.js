...
const JobsMixin = {
    getUserRow(canvas_user_id) {
        const userRows = this.usersRows.filter((thisRow) => thisRow.canvas_user_id === canvas_user_id)
        const nonExternalUserRows = userRows.filter((thisRow) => !this.isExternalUser(thisRow))
    ...
    getAccountRow(canvasAccountId) {
        return this.accountsRows.find((thisRow) => thisRow.canvas_account_id === canvasAccountId)
    },
    ...
    getSubaccountPath(subaccount) {
        ...
        if(!this.canTraverseUpTree(subaccountParent1)) return subaccountPath.join(', ')

        const subaccountParent2 = this.getAccountRow(subaccountParent1.canvas_parent_id)
        subaccountPath.unshift(subaccountParent2.name)
        return subaccountPath.join(', ')
    },

    getChildAccountIds(canvasAccountId) {
        return this.accountsRows.filter((thisRow) => thisRow.canvas_parent_id === canvasAccountId)
    },
    ...
    countCoursesOnSubaccount(canvasAccountId) {
        ...
        return count + this.coursesRows.filter((thisRow) => thisRow.canvas_account_id === canvasAccountId).length
    },

    getLccsOrUnitAdminsRows(accountRow) {
        return this.adminsRows.filter((thisRow) => thisRow.canvas_account_id === accountRow.canvas_account_id && SUBACCOUNT_ADMIN_ROLES.includes(thisRow.role_id))
    },

    canTraverseUpTree(accountRow) {
        return ![this.rootAccountId, this.accountId].includes(accountRow.canvas_parent_id)
    },
    ...
    getLccsOrUnitAdmins(canvasAccountId) {
        ...
        while(lccsUnitAdminsRows.length === 0 && accountRow && this.canTraverseUpTree(accountRow)){
            accountRow = this.getAccountRow(accountRow.canvas_parent_id)
            lccsUnitAdminsRows = this.getLccsOrUnitAdminsRows(accountRow)
        }

        const lccsUnitAdminsIds = lccsUnitAdminsRows.map((thisRow) => thisRow.canvas_user_id)
        ...
        const fromParentRecord = accountRow && accountRow.canvas_account_id !== canvasAccountId && lccsUnitAdminsRows.length > 0
        return {lccsOrUnitAdmins: lccsUnitAdminsNames.join(', '), lccsFromParentRecord: fromParentRecord}
    },

    getLastLogin(canvas_user_id) {
        const lastLogin = this.lastAccessRows.find((thisRow) => thisRow.user_id === canvas_user_id).last_access_at
        if(!lastLogin) return ''
    ...
    countAdminsForSubaccount(canvasAccountId) {
        return this.adminsRows.filter((thisRow) => thisRow.canvas_account_id === canvasAccountId).length
    }
}
