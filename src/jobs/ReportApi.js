...
class ReportApi {
  ...
  runReport = async (reportType, parameters = {}, options = {}) => {
    ...
    const { account, interval, reportAge, timeout } = {...this.#defaultOptions, ...options}
    let report
    ...
    if (reportAge > 0) {
      report = await fetch(`${host}/api/v1/accounts/${account}/reports/${reportType}`, {
        ...
        .then(runs => {
          let newest = runs
            ...
            // Find the newest one.
            .reduce((newest, run) => Date.parse(run.created_at) > Date.parse(newest.created_at) ? run : newest, { created_at: 0 })
    ...
    report = await fetch(`${host}/api/v1/accounts/${account}/reports/${reportType}`, {
      ...
      body: JSON.stringify({ parameters: {...parameters, 'skip_message': true }})})
      .then(checkOk)
      .then(r => r.json())
    ...
    do {
      ...
      report = await fetch(`${host}/api/v1/accounts/${account}/reports/${reportType}/${reportId}`, {
        headers: { 'Authorization': 'Bearer ' + token }
      })
        .then(checkOk)
        .then(r => r.json())
    ...
    if (report.status !== 'complete') {
      throw new Error('Report finished but it is not complete, id: ' + reportId)
    }
    return report
  }

  fetchReport(report) {
    return fetch(report.attachment.url)
      .then(checkOk)
  }
