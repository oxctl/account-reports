...
class SubaccountAdminsJob {
  ...
  run = async () => {
    try {
      ...
      const [adminsReport, usersReport, accountsReport, coursesReport] =
        await Promise.all([
          reportApi.runReport(
            "provisioning_csv",
            { admins: "true" },
      ...
      [this.adminsRows, this.usersRows, this.accountsRows, this.coursesRows] =
        await Promise.all([
      ...
      for (const row of Object.values(this.accountsRows)) {
        ...
        const coursesOnSubaccount =
          this.countCoursesOnSubaccount(canvasAccountId);
        const { lccsOrUnitAdmins, lccsFromParentRecord } =
          this.getLccsOrUnitAdmins(canvasAccountId);

        const countAdminsForSubaccount =
          this.countAdminsForSubaccount(canvasAccountId);

        output.push({
          "Subaccount Name": subaccountName,
          "Subaccount Path": subaccountPath,
          "LCC/Unit Admin for Subaccount": lccsOrUnitAdmins,
          "LCCs/Unit Admins from parent account?": lccsFromParentRecord
            ? "Yes"
            : "No",
          "Courses on Subaccount": coursesOnSubaccount,
          "Subaccount URL": subaccountUrl,
          "Number of users with account roles for the subaccount":
            countAdminsForSubaccount,
        });
      }
