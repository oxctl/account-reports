  ...
  run = async () => {
    try {
      ...
      for (const token of Object.values(tokenRows)) {
        ...
        const createdAt =
          token.creation && token.creation !== "never" ? new Date(token.creation) : null
        const expiresAt =
          token.expiration && token.expiration !== "never" ? new Date(token.expiration) : null

        if (createdAt?.getTime() > oneYearAgo.getTime()) reasons.push("Created within last year")
        if (expiresAt?.getTime() > oneYearAgo.getTime()) reasons.push("Expires over a year from now")
      ...
      const output = Object.values(this.adminsRows)
        .filter(admin => String(admin.canvas_account_id) === String(accountId))
        .flatMap(admin => {
